...
public class HIPIEPerspective implements IPerspectiveFactory {
	...
	public void createInitialLayout(IPageLayout layout) {
		...
		IFolderLayout left = layout.createFolder("left", IPageLayout.LEFT,
				(float) 0.26, layout.getEditorArea());
		left.addView(IPageLayout.ID_PROJECT_EXPLORER);
		IFolderLayout bot_left = layout.createFolder("bot_left",
				IPageLayout.BOTTOM, (float) 0.5,
				IPageLayout.ID_PROJECT_EXPLORER);
		bot_left.addView(IPageLayout.ID_OUTLINE);
		layout.addPlaceholder(DesignModeView.ID, IPageLayout.RIGHT, 0.6f,
				layout.getEditorArea());
		IFolderLayout bot = layout.createFolder("bot", IPageLayout.BOTTOM,
